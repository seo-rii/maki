...
- On a cleared allocation bit, only an all-zero slot header is "unwritten": a hole is never partly written, so an undecodable non-zero header on a cleared slot is damage ⇒ EIO, never zeros (S-01 residual). A shard data file is created at its full sparse size and synced before use, so a file shorter than `units_per_shard × slot_size` is *truncated*, and a slot beyond its end is damage too, not a hole. That evidence is volatile: the next slot write past the end grows the file with zeros and the removed slots become "holes", so `open` marks them allocated (EIO until rewritten) and the marks are persisted before the file is grown back. The one legitimate short file belongs to a shard never checkpointed into (uncataloged, empty allocation map; a stale empty copy from an earlier creation attempt is *not* proof of use): its holes stay holes, and its size is set and synced again before the shard is written to or cataloged, because after a failed sync and a restart the page cache shows a full size the disk does not have (K-01 again). Damaging the newest map copy *and* the slot header together is exactly what media damage after a crash looks like; the media-damage sweep in `review_r3b_durability.rs` finds this class in seconds.
