...
| Runtime logging (R4-001) | `maki-core/src/logging.rs` child-process tests and `maki-nbdkit/tests/review_r4_native_logging.rs`: a real nbdkit run must show the store's `WARN` for an adopted orphan shard on its captured stderr, and `MAKI_LOG=error` must suppress it |
| Guarded plaintext and keys (R4-002) | `maki-crypto-local/tests/review_r4_guarded_buffers.rs`: encryption, decryption, tampered-ciphertext failure and file/env key loading never wrap a bare `Vec` (`unguarded_wraps()` stays constant); `SecretBuffer::truncate` unit tests |
| Package removal and inputs (R4-003, R4-007) | `scripts/test_debian_package.py`: `prerm` refuses removal under an attachment record or an active `maki@`/`maki-attach@`/`maki-workload@` unit but never blocks upgrades, no maintainer script starts/stops/restarts a service, ELF stubs of the wrong machine, class, type or a non-ELF file are refused, unknown architectures are refused, `--shlibdeps` fails closed on unanalysable artifacts |
| Deep-check verdict (R4-004) | `maki-core/tests/review_r4_check_verdict.rs`: recoverable slot damage passes with a warning and is repaired by recovery, damage without a journal source fails, mixed damage reports both classes, a healthy volume is `clean` |
| Overlay memory bound (R4-005) | `maki-core/tests/review_r4_overlay_bound.rs`, `maki-format/tests/review_r4_overlay_limits.rs`, `maki-nbdkit/tests/review_r4_overlay_options.rs`: bytes and entries stay under their limits without journal pressure, the worker checkpoints on the overlay watermark with a manual clock, a failed reclaim refuses the write and degrades, defaults/zero/minimum validation, and the daemon mapping |
| Cache-first reads (R4-006) | `maki-core/tests/review_r4_cache_reads.rs`: a hit reads at most the 64-byte header and makes no provider call, an overlay hit reads nothing, newer versions miss, header damage is EIO despite a cached entry, payload damage is served until eviction then EIO, uncached reads still verify |
