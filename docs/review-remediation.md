...
| R4-002 (guarded plaintext) | `local-aes-gcm-siv` decrypted with the allocating AEAD API into an ordinary `Vec` and wrapped it in `SecretBuffer` afterwards; XTS copied ciphertext through a plain `Vec`; encryption copied plaintext into an unlocked vector before encrypting; key files were read into a plain `Vec` and hex-decoded into another. `secure-buffers` therefore did not cover plaintext or keys during their first moments. | The guarded buffer is allocated first and the AEAD/XTS operation runs in place (`encrypt_in_place_detached` / `decrypt_in_place_detached`, `encrypt_area` / `decrypt_area` on `expose_mut()`); key files are read straight into a `SecretBuffer` and hex-decoded into one; `SecretBuffer::from_vec` now counts `unguarded_wraps()`; `SecretBuffer::truncate` added; the `aes` crate's `zeroize` feature erases key schedules on drop. Residual: the expanded key schedule is not page-locked under `secure-buffers` ([transport memory](transport-memory.md#local-providers-and-key-material)). | `review_r4_guarded_buffers.rs` (maki-crypto-local: encrypt, decrypt, tampered-ciphertext failure and file/env key loading must not move `unguarded_wraps()`), `truncate_keeps_the_allocation_and_erases_the_tail`, `guarded_constructors_are_not_counted_as_unguarded_wraps` (maki-crypto) |
| R4-003 (package removal) | The package had `postinst` and `postrm` only. `apt remove maki` under a live attachment deleted `maki-attach`, the very program `ExecStop=` and `maki-recover@` need to detach, leaving no normal cleanup path. | A `prerm` refuses `remove`/`deconfigure` while a trusted attachment record exists under `/run/maki-attach/` or a `maki@`, `maki-attach@` or `maki-workload@` unit is active, and prints the drain-then-deactivate procedure. `upgrade` is exempt and no maintainer script starts, stops or restarts a service. | `test_prerm_refuses_removal_while_a_volume_is_attached`, `test_prerm_refuses_removal_while_a_maki_unit_is_active`, `test_prerm_allows_idle_removal_and_never_blocks_an_upgrade`, and the no-`systemctl start/stop/restart` scan over every maintainer script (`scripts/test_debian_package.py`) |
| R4-007 (package inputs) | `build-deb.py` checked only the *format* of `--architecture`; x86-64 artifacts requested as `arm64` produced a valid-looking arm64 package (reproduced by the reviewer), and `Depends` never reflected the binaries' native libraries. | Every release file's ELF header (machine, class, byte order, type) must match the requested architecture; the plugin must be `ET_DYN` and the binaries executables; unknown architectures are refused. `--shlibdeps` runs `dpkg-shlibdeps` and appends the result to `Depends`, failing closed when the scan cannot analyse an artifact. The package tests now use ELF stubs instead of text files. | `test_package_architecture_must_match_the_elf_binaries`, `test_release_files_must_be_elf_images_of_the_right_kind`, `test_unknown_debian_architecture_is_refused_before_building`, `test_shlibdeps_option_fails_closed_on_artifacts_it_cannot_analyse` |
| R4-004 (deep-check verdict) | `maki check --deep` reported every slot CRC failure as an error, although recovery rewrites a checkpointed slot from a validated journal record newer than the checkpoint. An operator could read a routinely recoverable volume as a data-loss incident. | The journal summary now also returns the set of units with an uncovered validated record (`ReplayRecords::Units`, one `u64` per unit, no payloads). A failing slot whose unit is in that set is reported as `warning: slot: recoverable: …` and does not fail the check; other damage stays an error. The report ends with `deep check verdict: clean | recoverable | unrecoverable` and counts both classes ([operations](operations.md#volume-lifecycle)). | `review_r4_check_verdict.rs` (maki-core): recoverable damage passes with a warning and is repaired by the next recovery, damage without a source fails, mixed damage reports both classes, a healthy volume is `clean` |
| R4-005 (overlay bound) | `limits.max_ciphertext_bytes` only bounds the remote scheduler's pending queue (local providers bypass it), while the overlay that holds journaled, uncheckpointed ciphertext had no bound of its own: with a slow or failing checkpoint it could grow toward the multi-GiB `journal_max_bytes`, a disk budget that is not an instance's RAM budget. | New `limits.max_overlay_bytes` (default 256 MiB) and `limits.max_overlay_entries` (default 262144) reach `EngineLimits`. Admission charges a request at its eventual size (`max(bytes, 2 × latest_bytes) + 2 × payload`, since every unit ends up with a durable copy); a write that would exceed a bound syncs and checkpoints inline like the journal hard limit and returns ENOSPC after a failed reclaim; the worker checkpoints at half of either bound. The overlay tracks `latest_bytes` (sanitizer-checked). Validation requires the byte bound to hold one maximal request twice and the entry bound to hold one maximal request; `0` disables. Library callers keep the unbounded default. | `review_r4_overlay_bound.rs` (maki-core: bytes and entries stay under their limits through 96/40 writes without journal pressure, the worker checkpoints on the watermark with a manual clock, a failed reclaim refuses the write and degrades, zero keeps library callers unbounded), `review_r4_overlay_limits.rs` (maki-format defaults, zero, minimums), `review_r4_overlay_options.rs` (maki-nbdkit mapping) |
| R4-006 (cache-first reads) | `read_secret` called `read_ct` (slot header, full ciphertext, CRC) *before* consulting the plaintext cache, so a hit only saved the provider call: the cache was a decryption cache, not a read cache. | The read path first establishes the unit's current write sequence (`Volume::current_sequence`: overlay entry, else the 64-byte slot header through `SlotStore::slot_sequence`, which applies every header-level refusal of `read_slot`) and serves a `(unit, sequence)` hit without reading the payload; misses read and decrypt as before. Versioning is unchanged (a newer write changes the sequence). Documented trade-off: a hit does not re-verify the on-disk payload CRC of an already validated version until the entry is evicted; header damage still refuses the read and the deep check still finds payload damage. `CrashableBacking::read_bytes()` counts served read bytes for tests. | `review_r4_cache_reads.rs` (maki-core: a hit reads ≤ 64 bytes and no provider call, an overlay hit reads nothing, newer versions miss, header damage is EIO despite a cached entry, payload damage is served until eviction then EIO, no-cache reads still verify) |
