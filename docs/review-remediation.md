...
| M-012 reordered batch results undetected (WebSocket, positional HTTP) | Partial | HTTP batch layouts now require `item_index_path` at validation, so positional zip without a unit echo is no longer configurable. WebSocket item echo is still open. | `http_batch_layout_requires_unit_echo` (maki-format) |
| M-013 incomplete configuration validation; placebo settings | Partial | `validate_settings` and `validate_provider_sections` (see [Configuration](configuration.md#validation-rules)): positive counts and byte limits, finite ratios and probe rates, ordered delay and breaker ranges, batch targets within maxima, capability mode, bounded-error timing, NBD size ordering, cache and control values, memory-lock mode, provider-specific required and forbidden sections, endpoint URL parsing, and the `keyring` source refused. Still open: the `[security]` flags and `cache.lock_memory` are validated but not yet applied by the daemon (tracked below). | `review_config.rs` (maki-format): `zero_and_inverted_bounds_are_rejected` (29 cases), `capability_mode_and_availability_policy_are_checked`, `keyring_credential_source_is_refused`, `local_provider_requires_key_and_rejects_transport_sections`, `duplicate_or_empty_endpoint_names_are_rejected`, `defaults_still_validate` |
