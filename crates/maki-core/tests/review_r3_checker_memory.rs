...
fn store_with_maps(units_per_shard: u64, shards: &[(u64, AllocationMap)]) -> SlotStore {
    ...
    for (shard, map) in shards {
        ...
        ab.store(backing.as_ref(), &mut map).unwrap();
        // A cataloged shard's data file is always at its physical size: a
        // shorter one is truncation damage and marks its slots allocated.
        let data = backing.open(&layout::shard_data(*shard), true).unwrap();
